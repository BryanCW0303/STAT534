...
MC3_iter <- function(V, A_cur, B_cur, data, response) {
  ...
  nbd_prime <- get_valid_nbd_set(V, A_cur, data, response)
  
  A_prime <- nbd_valid[[index]]
  p_A_prime <- -getLogisticAIC(response, A_prime, data) - log(length(nbd_valid ))
  p_A <- -getLogisticAIC(response, A_cur, data) - log(length(nbd_prime))
